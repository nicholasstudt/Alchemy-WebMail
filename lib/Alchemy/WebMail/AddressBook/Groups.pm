...
sub _checkvals {
	...
	if ( ! is_text( $in->{name} ) ) {
		push( @err, ht_li( {}, 'Enter a name for this group.' ) );
	}
	else {
		my $oname;
		...
		if ( $count ) {
			if ( defined $oname ) {
				if ( $oname ne $in->{name} ) {
					push( @err, ht_li( {}, 'This group name is already in use.' ) );
				}
			}
			else {
				push( @err, ht_li( {},	'This group name is already in use.') );
...
sub do_main {
	...
	while ( my ( $id, $name ) = db_next( $sth ) ) {
		push( @lines, 	ht_tr(),
						ht_td( {}, ht_a( "$$k{rootp}/members/$id", $name ) ),
						ht_td( 	{ 'class' => 'rdta' },
								'[',
								ht_a( "$$k{rootp}/edit/$id", 'Rename' ), '|',
								ht_a( 	"$$k{rootp}/delete/$id", 
										'Delete' ), ']' ),
						ht_utr() );
	}
...
sub do_members {
	...
	push( @lines, 	ht_div( { 'class' => 'box' } ),
					...
					ht_td( {}, 
							( @in ) ? 	( '<p>Click on the address to
							remove it from the list</p>',
										'<ul>', @in, '</ul>' ) : 
					...
					ht_td( {}, 
							( @out ) ? 	(
										'<p>Click on the address to add it
										to the list</p>',
										'<ul>', @out, '</ul>' ) : 
