...
sub _checkvals {
	...
	if ( ! is_number( $in->{main_role} ) ) {
		push( @errors, 'Select if this should be the default role.'. ht_br() );
	}

	if ( ! is_text( $in->{role_name} ) ) {
		push( @errors, 'Enter a name for this role.'. ht_br() );
	}

	if ( ! is_email( $in->{reply_to} ) ) {
		push( @errors, 'A role must have a reply to address.'. ht_br() );
	}
...
sub _form {
	...
	for my $folder ( sort 	{ ( $a eq $$k{imap_inbox} ) ? -1 : $a cmp $b }
		...
		push( @folders, $folder, $k->inbox_mask( $folder ) );
	}
	...
	return( ht_form_js( $$k{uri} ),	
			...
			ht_td( { 'class' => 'dta' }, ht_input( 'role_name', 'text', $in ) ),
			ht_utr(),
...
sub do_edit {
	...
	if ( ! ( my @errors = roles_checkvals( $in ) ) ) {
		...
		while ( my( $main, $rname, $name, $reply, $save, 
					$sig ) = db_next( $sth ) ) {
		...
		if ( $r->method eq 'POST' ) {
			return( @errors, roles_form( $k, $in ) );
		}
		else {
			return( roles_form( $k, $in ) );
		}
	}
} # END $self->do_edit
